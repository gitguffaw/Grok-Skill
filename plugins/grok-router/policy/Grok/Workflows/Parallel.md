...
```bash
node plugins/grok-router/scripts/grok-companion.mjs exec --lanes login,billing,tests add auth
...
`--panel` on review uses the frozen lane list. exec and analyze need `--lanes` with named slices. Leaves get `--no-subagents`. Nested companion fan-out is refused (`GROK_ROUTER_NESTING`).
